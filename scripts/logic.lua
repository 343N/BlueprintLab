...
function ToTheLab(player_index)
    ...
    -- if space exploration is enabled, we want to cancel out the modifications they make to the god controller
    if (game.active_mods["space-exploration"] ~= nil) then
        player.force.manual_mining_speed_modifier = 10000000
    end

    -- if editor extensions is enabled, enable those items in the lab
    if (game.active_mods['EditorExtensions'] ~= nil) then
	...
	if settings.global["blueprint-lab-pause-game"].value and isSingleplayer() then
        freezeAllSurfaces()
        saveMapSettings()
        pauseMapEvolution()
    end
